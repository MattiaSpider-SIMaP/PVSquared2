...
#Ottengo la std sugli RMSE ottenuti in ogni ciclo per ogni modello
lr_RMSE_std = np.std(lr_errors)    
knn_RMSE_std = np.std(knn_errors) 
rf_RMSE_std = np.std(rf_errors)   
gb_RMSE_std = np.std(gb_errors)   
svr_RMSE_std = np.std(svr_errors)  
nn_RMSE_std = np.std(nn_errors)   
...
ABO_RMSE_std = [lr_RMSE_std, knn_RMSE_std, rf_RMSE_std, gb_RMSE_std, svr_RMSE_std, nn_RMSE_std]
...
fig,ax = plt.subplots(figsize=(8,6))
print('All but one validation RMSE scores for LR, KNN, RF, GB, SVR, NN,: ' , ABO_scores)
print('All but one validation RMSE std scores for LR, KNN, RF, GB, SVR, NN,: ' , ABO_RMSE_std)
# print('Std. dev: ',stds)
fontsize = 12
plt.rc('xtick', labelsize=fontsize)
plt.rc('ytick', labelsize=fontsize)
model = ['Linear Regression', 'K Nearest Neighbors', 'Random Forest', 'Gradient Boosting', 'SVR', 'Neural Network']
ax.bar(model, ABO_scores, yerr=ABO_RMSE_std, alpha=0.7, capsize=4)
ax.set_ylabel('RMSE (mV)',fontsize=fontsize)
# ax.set_title('All but One validation scores (experimental means)',fontsize=fontsize)
plt.xticks(rotation=90)
plt.ylim(0,70)
