...
# ProcessingData_cols = ["DeviceName", "Concentration(mM)", "Annealing_T(C)", "Molecular_Weight", "Heavy_Atom_Count", "Rigid_Bond_Count", "Rotatable_Bond_Count", "Topological_Polar_Surface_Area", "Complexity", "Hydrogen_Bond_Donor_Count", "Hydrogen_Bond_Acceptor_Count", "H_Count", "C_Count", "Cl_Count", "Br_Count", "I_Count", "Halide_fraction", "Heteroatom_carbon_ratio"]
...
# ProcessingData_cols = ["DeviceName", "Concentration(mM)", "Molecular_Weight", "Rigid_Bond_Count", "Rotatable_Bond_Count", "Complexity", "Cl_Count", "Br_Count", "I_Count", "Halide_fraction", "Heteroatom_carbon_ratio"]
...
with sns.axes_style("white"):
    ...
    ax = sns.heatmap(cor, annot=True, annot_kws={"size": 14}, fmt=".2f",
...
# X_train_stand_no = X_train_stand = X_train.drop("DeviceName", axis=1)
...
# column_names = ["Concentration(mM)", "Annealing_T(C)", "Molecular_Weight", "Rigid_Bond_Count", "Rotatable_Bond_Count", "Complexity", "Cl_Count", "Br_Count", "I_Count", "Halide_fraction", "Heteroatom_carbon_ratio"]

# # Imposta le etichette dell'asse x con i nomi delle colonne
# ax[1].set_xticks(np.arange(len(column_names)))
# ax[1].set_xticklabels(column_names, rotation=90, fontsize=10)  # Puoi regolare la rotazione per una migliore visualizzazione
# plt.show()
...
def prediction_vs_ground_truth_fig(y_train, y_train_hat, y_test, y_test_hat, model_name, letter):
    ...
    plt.xticks([1000, 1050, 1100, 1150, 1200])
    plt.yticks([1050, 1100, 1150])
    plt.tick_params(direction='in')
    plt.xlim([1000,1200]) 
    plt.ylim([1000,1200])
    plt.text(1005, 1195, '{} - Train RMSE: {:.2e}, Test RMSE: {:.2e}'.format(model_name, np.sqrt(metrics.mean_squared_error(y_train, y_train_hat)), np.sqrt(metrics.mean_squared_error(y_test, y_test_hat))), fontsize=fontsize, ha='left', va='top')
    plt.text(980, 1198, letter, fontsize=20, ha='left', va='top')
    b = plt.scatter(y_test, y_test_hat, s=25,c='#1f78b4')
    plt.legend((a,b),('Train','Test'),fontsize=15,handletextpad=0.1,borderpad=0.1, loc='lower right')
    plt.rcParams['font.family']="Arial"
    plt.tight_layout()
    plt.show()
    
# with experimental values means
...
#     plt.xlabel('Observation', fontsize=fontsize)
#     plt.ylabel('Prediction', fontsize=fontsize)
#     plt.xticks([1050, 1075, 1100, 1125, 1150])
#     plt.yticks([1075, 1100, 1125, 1150])
#     plt.tick_params(direction='in')
#     plt.xlim([1050,1160]) 
#     plt.ylim([1050,1160])
#     plt.text(1055, 1157, '{} - Train RMSE: {:.2e}, Test RMSE: {:.2e}'.format(model_name, np.sqrt(metrics.mean_squared_error(y_train, y_train_hat)), np.sqrt(metrics.mean_squared_error(y_test, y_test_hat))), fontsize=fontsize, ha='left', va='top')
#     plt.text(1035, 1160, letter, fontsize=20, ha='left', va='top')
#     b = plt.scatter(y_test, y_test_hat, s=25,c='#1f78b4')
#     plt.legend((a,b),('Train','Test'),fontsize=15,handletextpad=0.1,borderpad=0.1, loc='lower right')
#     plt.rcParams['font.family']="Arial"
#     plt.tight_layout()
...
letter = "a"
prediction_vs_ground_truth_fig(Y_Voc_train, Y_Voc_train_LR, Y_Voc_test, Y_Voc_test_LR, "LR", letter)
...
# Visualize the results
letter = "b"
prediction_vs_ground_truth_fig(Y_Voc_train, Y_Voc_train_KN, Y_Voc_test, Y_Voc_test_KN, "KNN", letter)
...
#Visualize the results
letter = "c"
prediction_vs_ground_truth_fig(Y_Voc_train, Y_Voc_train_RF, Y_Voc_test, Y_Voc_test_RF, "RF", letter)
...
# Visualize the results
letter = "d"
prediction_vs_ground_truth_fig(Y_Voc_train, Y_Voc_train_gb, Y_Voc_test, Y_Voc_test_gb, "GB", letter)
...
# Visualize the results
letter = "e"
prediction_vs_ground_truth_fig(Y_Voc_train, Y_Voc_train_svr, Y_Voc_test, Y_Voc_test_svr, "SVR", letter)
...
#Visualize the results
letter = "f"
prediction_vs_ground_truth_fig(Y_Voc_train, Y_Voc_train_MLP, Y_Voc_test, Y_Voc_test_MLP, "NN", letter)
...
# lr_d = cross_validate(lr_regressor, X_stand, Y_Voc, cv = n_cv, scoring='neg_mean_squared_error', return_estimator = True, return_train_score = True)
# knn_d = cross_validate(knn_regressor, X_stand, Y_Voc, cv = n_cv, scoring='neg_mean_squared_error', return_estimator = True, return_train_score = True)
# rf_d = cross_validate(rf_regressor, X_stand, Y_Voc, cv = n_cv, scoring='neg_mean_squared_error', return_estimator = True, return_train_score = True)
# gb_d = cross_validate(gb_regressor, X_stand, Y_Voc, cv = n_cv, scoring='neg_mean_squared_error', return_estimator = True, return_train_score = True)
# svr_d = cross_validate(svr_regressor, X_stand, Y_Voc, cv = n_cv, scoring='neg_mean_squared_error', return_estimator = True, return_train_score = True)
# nn_d = cross_validate(nn_regressor, X_stand, Y_Voc, cv = n_cv, scoring='neg_mean_squared_error', return_estimator = True, return_train_score = True)
...
# # Fit GB to all dataset 
# gb_regressor.fit(X_stand, Y_Voc)
...
# shap_values_GB = explainerGB.shap_values(X_stand)

# plt.figure()
# shap.summary_plot(shap_values_GB, X_noname, plot_type = "dot", plot_size=(12, 8),
#                             color=plt.get_cmap("plasma"),
#                             show = False)
# plt.rcParams['font.sans-serif'] = "Arial"
# plt.rcParams.update({'font.size': 60})
...
# # Fit SVR to all dataset 
# svr_regressor.fit(X_stand, Y_Voc)

# # Costruisci l'oggetto KernelExplainer
# svm_explainer = shap.KernelExplainer(svr_regressor.predict, X_stand)

# # Calcola gli SHAP values
# svm_shap_values = svm_explainer.shap_values(X_stand)
...
# # Use shap to explain KNN model results

# import shap
# import matplotlib.pyplot as plt

# # Fit SVR to all dataset 
# knn_regressor.fit(X_stand, Y_Voc)

# # Costruisci l'oggetto KernelExplainer
# knn_explainer = shap.KernelExplainer(knn_regressor.predict, X_stand)

# # Calcola gli SHAP values
# knn_shap_values = knn_explainer.shap_values(X_stand)

# # Plot degli SHAP values
# plt.figure(figsize=(12, 8))
# summary = shap.summary_plot(knn_shap_values, X_noname, plot_type="dot",
#                             color=plt.get_cmap("plasma"),
#                             show=False)
